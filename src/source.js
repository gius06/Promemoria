...
class Promemoria {
    ...
    */
    ...
    /**
     * Cerca le attività contenenti le parole chiave specificate.
    ...
    /**
     * @description - Modifica il nome e/o la data di un'attività esistente.
     ...
     * @returns {boolean} - True se l'attività è stata modificata con successo, altrimenti false.
     */
    modificaAttività(vecchiaNomeAttività, nuovoNomeAttività, nuovaDataAttività) {
        // Itera attraverso le categorie per trovare l'attività da modificare
        for (const [categoria, attivitàList] of this.mappa.entries()) {
            const indice = attivitàList.findIndex(a => a.nomeAttività === vecchiaNomeAttività);
            if (indice !== -1) {
                 // Se l'attività viene trovata, controlla se ci sono modifiche da fare
                if (nuovoNomeAttività) {
                    // Se è fornito un nuovo nome, lo assegna all'attività corrispondente
                    attivitàList[indice].nomeAttività = nuovoNomeAttività;
                }
                if (nuovaDataAttività) {
                    // Se è fornita una nuova data, la assegna all'attività corrispondente
                    attivitàList[indice].dataAttività = nuovaDataAttività;
                }
                // Restituisce true per indicare che l'attività è stata modificata con successo
                return true;
            }
        }
 ...
 * @param {Promemoria} promemoria - L'oggetto promemoria contenente le attività.
 */
    ...
    if (Array.from(risultati.keys()).length > 0) {
        ...
        if(attivitàDaModificare!==null){
            do {
                ...
                conferma = parseInt(prompt("                             > "));
                console.log();
                // Inizializza la variabile per il nome dell'attività.
                ...
                switch (conferma) {
                    ...
                    case 1: {
                        ...
                        // Modifica il nome dell'attività nel promemoria.
                        ...
                        console.log("                     ═══════════════════════════════════════════\n");
                        // Interrompe il ciclo se l'utente ha scelto di modificare solo il nome.
                        if(conferma!==3)break;
                    }
                    case 2: {
                        // Inizializza la variabile per la nuova data dell'attività.
                        let dataAttività;
                        let contatore = 0;
                        do{
                            // Richiede all'utente di inserire la nuova data.
                            contatore = 0;
                            console.log("                           ════════════════════════════════");
                            console.log("                           INSERISCI LA NUOVA DATA (D/M/YY)");
                            console.log("                           ════════════════════════════════\n");
                            dataAttività = formattaData();
                            // Conta quante attività hanno lo stesso nome e la stessa data.
                            ...
                            // Verifica se ci sono attività con lo stesso nome e la stessa data.
                            if(contatore>0)
                            {
                                console.clear();
                                console.log("\n ══════════════════════════════════════════════════════════════════════════════════");
                                console.log(" !!!ATTENZIONE, DUE ATTIVITÀ CON LO STESSO NOME NON POSSONO AVERE LA STESSA DATA!!!");
                                console.log(" ══════════════════════════════════════════════════════════════════════════════════\n");
                                // Visualizza il promemoria con le attività duplicate.
                                promemoria.stampaMappa(risultati);
                            }
                        }while(contatore>0);
                        // Modifica la data dell'attività nel promemoria.
                    ...
                    default: {
                        // Visualizza un messaggio di scelta non valida.
                        console.log("\n                                ═══════════════════════");
